"""
preprocessing/v1/clahe_AB.py

V1 Preprocessor: CLAHE on A and B channels of LAB colour space.

Algorithm:
    1. RGB → LAB  (cv2.COLOR_RGB2LAB)
    2. Split into L, A, B channels
    3. Apply CLAHE to A and B channels  (L untouched)
...
Parameters (from configs_ResNet/v1_AB.yaml):
...
class V1ABPreprocessor:
    """CLAHE on A and B channels only (L untouched).
    ...
    def apply(self, image: np.ndarray, label: np.ndarray = None):
        """Apply CLAHE to A and B channels only.
        ...
        lab_clahe = cv2.merge((l, a_clahe, b_clahe))
