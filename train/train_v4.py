...
Entry point for V4: V1 soft routing × V2 (LSE + Augmentation) × 5 losses.

Outer loop: 5 CLAHE variants (V0, V1_L, V1_LA, V1_LB, V1_LAB)
Inner loop: 5 loss functions

Total: 5 variants × 5 losses = 25 training runs.
After all runs, soft_route_uniform is applied per loss across 5 variant probs.
...
class ODIRV4TrainDataset(Dataset):
    """Training dataset for V4: loads image → V4Preprocessor.apply() → tensor.

    V4Preprocessor applies V1 CLAHE then V2 augmentation in sequence.
    LSE-expanded records are passed in (list of dicts).
    """
    ...
    def __init__(
        ...
        # Resize BEFORE CLAHE so V1 and V4 train both operate on 224×224
        self._resize = T.Resize((img_size, img_size))
        # ToTensor + Normalize applied after preprocessor in train mode
...
class ODIRV4ValDataset(Dataset):
    """Validation dataset for V4: loads image → V1Preprocessor(variant) → tensor.

    No V2 augmentation on validation (guardrail from spec).
    V1 CLAHE is applied (is_train_only() == False for V1).
    """
...
# ---------------------------------------------------------------------------
# Training loop (standard, non-Mixup — V4 uses V2 aug, not V3 Mixup)
# ---------------------------------------------------------------------------

def train_one_epoch(model, loader, optimizer, criterion, device, scaler=None):
    """Standard (non-Mixup) training epoch for V4."""
    model.train()
    total_loss = 0.0
...
def train(cfg: dict, smoke: bool = False) -> None:
    ...
    # Val records built from val_df rows (no LSE on val, ever)
    val_records = [
        {
    ...
    # Accumulators for routing aggregation
    # probs_by_loss_by_variant[loss_name][variant] = np.ndarray (N, C)
    probs_by_loss_by_variant: Dict[str, Dict[str, np.ndarray]] = {
        ln: {} for ln in loss_experiments
    }
    # per_variant_per_loss_metrics[variant][loss_name] = metrics dict
    ...
    for v_idx, variant in enumerate(variants, start=1):
        ...
        # ---- Datasets (recreated each variant; records reused) ----
        train_ds = ODIRV4TrainDataset(
            expanded_records,
            image_dir=image_dir,
            img_size=img_size,
            preprocessor=v4_pre,
        ...
        val_ds = ODIRV4ValDataset(
            val_records,
            image_dir=image_dir,
            img_size=img_size,
            preprocessor=v1_pre,    # V1 only — no V2 on val (guardrail)
            smoke_n=smoke_val,
        )
        ...
        for loss_idx, loss_name in enumerate(loss_experiments, start=1):
            ...
            # Fresh model from identical init (re-seed before each run)
            seed_everything(seed)
            model = build_model(
                name=cfg["model_name"],
            ...
            # ---- Post-training: reload best, TTA eval, thresholds ----
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train V4: V1 × V2 × 5 losses")
